...
export class StudentTable extends Component {
    ...
    newStudent() {
      ...
      openEditStudent = (id) => {
        StudentDataService.getbyid(id).then(res => {
            this.setState({
                dataSelected: res,
                visibleRight: true,
                add:false
        ...
        return (
            <div className="p-grid">
                <Growl ref={(el) => this.growl = el} />
                  <Button icon="pi-md-close" className="p-button-danger" style={{ 'float': 'right' }} onClick={(e) => this.deleteNotify.delete(1359044)} />
                ...
                <div className="p-col-12">
                    {this.state.isAdmin ?
                        <div className="card card-w-title datatable-demo">
                            ...
                            <DataTable value={this.state.students} selectionMode="single" header="Students List" paginator={true} rows={10}
                                ...
                                <Column field="student_id" header="Student ID"  sortable={false}  />
            ...
            <div>
            <Sidebar visible={this.state.visibleRight} position="right" baseZIndex={1000000} onHide={(e) => this.setState({visibleRight: false})} style={{width: '30%'}}>
                <h1 style={{fontWeight:'normal'}}>Edit Student</h1>
                    <div>
                        <h3>Student ID</h3><br/>
                        <InputText value={this.state.dataSelected.student_id} onChange={this.onChangeStudentID} id="student_id" name="student_id" /> <br/>
                        <h3>Student Name</h3><br/>
                        <InputText value={this.state.dataSelected.name} onChange={this.onChangeStudentName} id="name" name="name" /> <br/>
                        <h3>Class ID</h3><br/>
                        <InputText value={this.state.dataSelected.class_id} onChange={this.onChangeClassID} id="class_id" name="class_id" /> <br/>
                    </div>
                <br/>
                <Button type="button" onClick={this.saveStudent} label="Submit" className="p-button-success"  style={{marginRight:'.25em'}} />
                <Button type="button" onClick={(e) => this.setState({visibleRight: false})} label="Close" className="p-button-secondary"/>
