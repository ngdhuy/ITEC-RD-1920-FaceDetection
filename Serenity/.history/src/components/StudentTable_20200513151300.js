...
export class StudentTable extends Component {
    ...
    componentDidMount(e) {
        this.StudentService.getStudent()
            .then(res => res.data)
            .then(data => { this.setState({ student_id: data.student_id }) })
            .catch(error => {
                if (error) {
                    this.setState({
                        isAdmin: false
                ...
                <div className="p-col-12">
                    {this.state.isAdmin ?
                        <div className="card card-w-title datatable-demo">
                            ...
                            <DataTable value={this.state.student_id} ref={(el) => this.dt = el} selectionMode="single" header="Students List" paginator={true} rows={10}
                                responsive={true} >
                                
                                <Column field="student_idd" header="Student ID" sortable={true} value={this.state.students} />
